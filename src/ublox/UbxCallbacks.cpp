...
{
    callbacks_[UBX_ACK_ACK] = std::bind(
    ...
    callbacks_[UBX_ACK_NAK] = std::bind(
        &UbxCallbacks::ackNakCb,
        this,
        std::placeholders::_1,
        EUbxMsg::UBX_ACK_NAK,
    ...
    callbacks_[UBX_CFG_CFG] = [](ubxmsg::IUbxMsg& ubxMsg) -> void {
        // empty callback
    };

    callbacks_[UBX_CFG_GEOFENCE] =
    ...
    callbacks_[UBX_CFG_MSG] = [&configRegistry](ubxmsg::IUbxMsg& ubxMsg) -> void {
    ...
    callbacks_[UBX_CFG_NAV5] = [&configRegistry](ubxmsg::IUbxMsg& ubxMsg) -> void {
    ...
    callbacks_[UBX_CFG_PRT] = [&configRegistry](ubxmsg::IUbxMsg& ubxMsg) -> void {
    ...
    callbacks_[UBX_CFG_RATE] = [&configRegistry](ubxmsg::IUbxMsg& ubxMsg) -> void {
        configRegistry.isRateCorrect(
            ubxMsg.serialize() == configRegistry.rateConfig().serialize()
        );
    };

    callbacks_[UBX_CFG_TP5] = [&configRegistry](ubxmsg::IUbxMsg& ubxMsg) -> void {
        configRegistry.isTimepulseConfigCorrect(
            ubxMsg.serialize() == configRegistry.timepulseConfig().serialize()
        );
    };

    callbacks_[UBX_CFG_VALSET] = [](ubxmsg::IUbxMsg& ubxMsg) -> void {
        // empty callback
    };

    callbacks_[UBX_CFG_VALGET] = [&configRegistry](ubxmsg::IUbxMsg& ubxMsg) -> void {
    ...
    IUbloxConfigRegistry& configRegistry)
{
    if (eUbxMsg == EUbxMsg::UBX_ACK_ACK)
    {
        ...
        if (eUbxMsgFromNakOrAck != END_UBX)
        {
